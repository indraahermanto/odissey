...
class M_task extends CI_Model {
  ...
  function saveReview($task, $data){
    $this->db->where('task_id', $task->task_id);
    $update = $this->db->update('od_task_doc', $data);
    if($update){
      logger('revTaskSucc', implode('; ', $data));
      docLog($task->task_id, 'task', 'rev', implode(';', $data));
      return true;
    }else{
      logger('taskRevFail', implode('; ', $data));
      return show_error("Error: Revision Task Fail. <br/>Something Wrong, please contact administrator.");
    }
  }
}