...
class PDF extends FPDF{
	public $report;
	public function setData($report, $partner){
    $this->report = $report;
    ...
    $this->cell(150,6,"BERITA ACARA SETTLEMENT",0,1,'C',1);
    $this->setFont('Times','',14);
    $this->cell(150,6,"LAYANAN ".strtoupper($this->report->name),0,1,'C',1);
    $this->setFont('Times','',12);
    $this->cell(150,6,"Antara PT Telekomunikasi Indonesia, Tbk dengan ".ucwords($this->report->partner_name),0,1,'C',1);
    $this->cell(150,6,"Nomor: ".$this->report->number,0,1,'C',1);
    $this->cell(150,6,"Tanggal: ".$this->report->created,0,1,'C',1);
    $line = 180;
	...
	function Content(){
		...
		$this->MultiCell(130,6,"Pada Hari ini ".$this->report->hari_body." Tanggal ".date('d', strtotime($this->report->ba_created))." Bulan ".$this->report->bulan_body." Tahun ".date("Y", strtotime($this->report->ba_created)).", dilaksanakan rekonsiliasi data layanan ".ucwords($this->report->name)." antara ".''." dengan Telkom dengan hasil sebagai berikut:",0,'J');

		$this->Ln(5);
		$this->SetLeftMargin(45);
		$this->setFont('Times','B',10);
		$this->Cell(5,6,"1. ");
		$this->MultiCell(115,6,"Hasil transaksi ".strtoupper($this->report->name)." untuk periode ".$this->report->periode." sejumlah Rp. ".number_format($this->report->ba_amount, 0, ',', '.').",- (".$this->report->terbilang." Rupiah) dengan rincian transaksi terlampir dibawah.",0,'J');
		
		// $header = array('Nama Publisher', 'Penjualan', 'Share');
		$this->Table();

		$this->Cell(5,6,"2. ");
		$this->MultiCell(115,6,ucwords($this->report->partner_name)." menerbitkan tagihan kepada Telkom sejumlah Rp. "
  							.number_format($this->report->ba_amount_pks, 0, ',', '.')
                .",- (".$this->report->terbilang_pks." Rupiah), belum termasuk PPn 10%.",0,'J');

		$this->SetLeftMargin(40);
		// $this->Cell(5,6,"",1,1,'L',1);
		$this->Ln(5);
		$this->setFont('Times','',11);
		$this->MultiCell(130,6,"Demikian Berita Acara ini dibuat untuk digunakan sebagai dasar pembayaran oleh Telkom kepada ".ucwords($this->report->partner_name)." dan apabila dikemudian hari ditemukan kekeliruan dalam perhitungan data ini, maka akan dilakukan perbaikan pada settlement berikutnya.",0,'J');
		// ""
		// foreach ($data as $key) {
  ...
  //   }
    ...
    $this->SetLeftMargin(35);
		$this->Ln(1);
    $this->Cell(40,12,"Nama Publisher",1,0,'C',1);
    $this->Cell(50,6,"Penjualan",1,0,'C',1);
    $this->Cell(50,6,"Share",1,1,'C',1);
    $this->SetLeftMargin(75);
    $this->Cell(20,6,"Total",1,0,"C",1);
    $this->Cell(30,6,"Rupiah",1,0,'C',1);
    $this->Cell(25,6,"Telkom ".$this->partner->pks_telkom_share."%",1,0,'C',1);
    $this->Cell(25,6,"Mitra ".$this->partner->pks_mitra_share."%",1,1,'C',1);
    $this->SetLeftMargin(35);
    $this->Ln(0);
    $this->MultiCell(40,6,strtoupper($this->report->partner_name),1,'C');
    $this->SetLeftMargin(75);
    $this->Ln(-6);
    $this->MultiCell(20,6,number_format(sizeof($this->report->orders), 0, ',', '.'),1,'C');
    $this->SetLeftMargin(95);
    $this->Ln(-6);
    $this->MultiCell(30,6,number_format($this->report->ba_amount, 0, ',', '.'),1,'C');
    $this->SetLeftMargin(125);
    $this->Ln(-6);
    $this->MultiCell(25,6,number_format($this->report->telkom_share, 0, ',', '.'),1,'C');
    $this->SetLeftMargin(150);
    $this->Ln(-6);
    $this->MultiCell(25,6,number_format($this->report->mitra_share, 0, ',', '.'),1,'C');
   ...
   //  }
	...
	function Approval($mgr_st){
		...
		$this->Ln(10);
		$this->setFont('Times','',11);
		$this->MultiCell(60,6,"PT TELEKOMUNIKASI INDONESIA",0,'C');
		// $this->SetCol(2);
		$this->Ln(-6);
		$this->SetLeftMargin(110);
		$this->MultiCell(60,6,strtoupper($this->report->partner_name),0,'C');
		
		
		$this->SetLeftMargin(40);
		$this->setFont('Times','U',11);
		$this->Ln(30);
		$this->MultiCell(60,6,strtoupper(ucwords($mgr_st->first_name." ".$mgr_st->last_name)),0,'C');
		$this->setFont('Times','',11);
		$this->MultiCell(60,6,ucwords("Mgr. Settlement & Business Analyst"),0,'C');
		$this->Ln(-12);
		$this->SetLeftMargin(110);
		$this->setFont('Times','U',11);
		$this->MultiCell(60,6,ucwords($this->report->first_name." ".$this->report->last_name),0,'C');
	}
...
$report->created 		= $this->convnumber->indonesian_date(strtotime($report->ba_created), 'd F Y','');
$report->hari_body 	= $this->convnumber->indonesian_date(strtotime($report->ba_created), 'l','');
$report->bulan_body	= $this->convnumber->indonesian_date(strtotime($report->ba_created), 'F','');
$report->periode 		= $this->convnumber->indonesian_date(strtotime($period[0]), 'd F Y', '')." sampai ".$this->convnumber->indonesian_date(strtotime($period[1]), 'd F Y', '');
$report->telkom_share = $report->ba_amount*($partner->pks_telkom_share/100);
$report->mitra_share  = $report->ba_amount*($partner->pks_mitra_share/100);
$report->total        = $report->ba_amount-$report->telkom_share;
$report->terbilang 		= $this->convnumber->terbilang($report->ba_amount);
$report->terbilang_pks 	= $this->convnumber->terbilang($report->ba_amount_pks);
$report->orders			= $orders;
// echo $report->created;
$pdf->SetTitle("Berita Acara ".$report->number);
// $pdf->SetSubject("Berita Acara ".$report->number);
$pdf->setData($report, $partner);
$pdf->AliasNbPages();
$pdf->AddPage();
$pdf->Content();
...
$pdf->Approval($mgr_st);
$pdf->Output("I","Berita Acara ".$report->number.".pdf");
